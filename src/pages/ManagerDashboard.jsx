...
//                     <Card.Text className="text-center">
//                       Create new manager or receptionist accounts for this system.
//                     </Card.Text>
      ...
      const totalSales = items.reduce(
        (sum, item) =>
          ...
          parseFloat(item.sellingPrice || 0) *
            parseFloat(item.quantityInStock || 0),
        0
      );
      const lowStock = items.filter(
        (item) => (item.quantityInStock || 0) < 5
      ).length;
      setStats({ totalItems, totalSales, lowStock });
    } catch (err) {
      console.error("Error fetching stats:", err);
    ...
    <>
        ...
        <Container>
          ...
          <Navbar.Collapse id="basic-navbar-nav">
            <Nav className="ms-auto">
              <Nav.Link onClick={() => navigate("/")}>Home</Nav.Link>
              {role === "MANAGER" && (
                <Nav.Link onClick={() => navigate("/inventory")}>
                  Inventory
                </Nav.Link>
              )}
              {role === "MANAGER" && (
                <Nav.Link onClick={() => navigate("/salesList")}>
                  Sales List
                </Nav.Link>
              )}
              {role === "MANAGER" && (
                <Nav.Link onClick={() => navigate("/reportPage")}>
                  Profit Report
                </Nav.Link>
              )}
              {role === "MANAGER" && (
                <Nav.Link onClick={() => navigate("/saleForm")}>
                  Record Sale
                </Nav.Link>
              )}
              {role === "MANAGER" && (
                <Nav.Link onClick={() => navigate("/itemPage")}>
                  Item Page
                </Nav.Link>
              )}
      ...
      >
        <Container>
          ...
          <p className="text-center text-muted mb-5">
            Welcome to the Sales Dashboard! Manage inventory, monitor sales, and
            gain insights into your business performance. This dashboard helps
            you efficiently manage inventory, monitor sales and profits, and
              ...
              <Col xs={12} md={4}>
                <Card className="shadow-sm h-100 hover-shadow">
                  <Card.Body className="d-flex flex-column justify-content-center align-items-center">
                    ...
                    <Card.Title>Manage Users</Card.Title>
                    <Card.Text className="text-center">
                      Create new manager or receptionist accounts for this
                      system.
                    </Card.Text>
                    <Button
                      variant="warning"
